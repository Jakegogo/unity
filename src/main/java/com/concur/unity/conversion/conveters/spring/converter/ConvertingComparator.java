...
package com.concur.unity.conversion.conveters.spring.converter;

import com.concur.unity.conversion.conveters.spring.ComparableComparator;
import com.concur.unity.conversion.conveters.spring.ConversionService;
import com.concur.unity.utils.Assert;
...
public class ConvertingComparator<S, T> implements Comparator<S> {
	...
	/**
	 * Create a new {@link ConvertingComparator} that compares {@link Entry
	 * map * entries} based on their {@link Entry#getKey() keys}.
	 *
	 * @param comparator the underlying comparator used to compare keys
	 * @return a new {@link ConvertingComparator} instance
	 */
	public static <K, V> ConvertingComparator<Entry<K, V>, K> mapEntryKeys(
			Comparator<K> comparator) {
		return new ConvertingComparator<Entry<K,V>, K>(comparator, new Converter<Entry<K, V>, K>() {
	...
	/**
	 * Create a new {@link ConvertingComparator} that compares {@link Entry
	 * map entries} based on their {@link Entry#getValue() values}.
	 *
	 * @param comparator the underlying comparator used to compare values
	 * @return a new {@link ConvertingComparator} instance
	 */
	public static <K, V> ConvertingComparator<Entry<K, V>, V> mapEntryValues(
			Comparator<V> comparator) {
		return new ConvertingComparator<Entry<K,V>, V>(comparator, new Converter<Entry<K, V>, V>() {
